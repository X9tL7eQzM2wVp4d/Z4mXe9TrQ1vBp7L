...
function mouseEntered()
	-- S'assurer qu'aucun curseur custom n'est présent
	...
	-- Utiliser le curseur de base
	UserInputService.OverrideMouseIconBehavior = Enum.OverrideMouseIconBehavior.None

	-- Garder la logique de détection de resize
	RunService:BindToRenderStep("SIMPLESPY_CURSOR", 1, function()
		if mouseInGui and _G.SimpleSpyExecuted then
			...
			if inRange and not sideClosed and not closed then
				-- ici tu peux ajouter des effets visuels (bordures, effets de survol etc.)
				-- mais on n'utilise plus de custom cursor
			end
		else
