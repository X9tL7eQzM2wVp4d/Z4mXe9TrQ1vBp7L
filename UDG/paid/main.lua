...
local tabs = {
    ["player"] = window:tabs({name = "player", icon = "http://www.roblox.com/asset/?id=134457279244886", size = udim2_new(0, 35, 0, 35)}),
        ...
        do -- UI
            ...
            do -- misc
                do -- menu
                    ...
                    debug:toggle({name = "AutoRebirth", description = "Enable AutoRebirth", default = false, flag = "keybind_autorebirth"});
                end;
            end;
            do -- player
                do -- menu
                    local player = tabs["player"]:section({name = "player", side = "left"});
                    player:slider({name = "PlayerSpeed", min = 16, max = 300, default = 16, decimals = 1, suffix = "m/s", flag = "player_PlayerSpeed", callback = 
                        function(state) 
                            local plr = game.Players.LocalPlayer.Name
                            game.Workspace[plr].Humanoid.WalkSpeed = state
                        end
                    });
                    player:keybind({name = "InfJump", description = "Put a key", flag = "player_InfJump", mode = "Toggle", usekey = false, callback = 
                        function(state)
                            _G.infinjump = state
                    ...
                    player:keybind({name = "Noclip", description = "Put a key", flag = "player_noclip", mode = "Toggle", usekey = false});
                end;
            end;
        end;
        --
            ...
            do -- connections
                framework.modules.signals.connection(run_service["RenderStepped"], LPH_NO_VIRTUALIZE(function()
                    ...
                    do -- the stuff
                        ...
                        do -- autorebirth
                            if flags["keybind_autorebirth"] then
